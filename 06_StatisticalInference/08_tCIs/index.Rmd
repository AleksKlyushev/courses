...
In R typing `data(ep)` brings up the ep data originally
...
library(ggplot2)
g <- ggplot(ep, aes(x = group, y = extra, group = factor(ID)))
...
## Results
```{r, echo=TRUE}
g1 <- ep$extra[1 : 10]; g2 <- ep$extra[11 : 20]
difference <- g2 - g1
...
t.test(extra ~ I(relevel(group, 2)), paired = TRUE, data = ep)
```
...
as.vector(t.test(extra ~ I(relevel(group, 2)), paired = TRUE, data = ep)$conf.int)
)
```
...
---
## Mistakenly treating the ep data as grouped
```{r}
